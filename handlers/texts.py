...
def translate(message, lang_from, lang_to):
    ...
    translator_text = translator.translate(message.text, dest=code_to, src=code_from).text
    _id = add_translation(message.text, translator_text, code_from, code_to, chat_id) #dobavlenie perevoda v bd
    bot.send_message(chat_id, f'<i>{translator_text}</i>', parse_mode='HTML', reply_markup=fav_button(_id))
